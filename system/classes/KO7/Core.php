...
class KO7_Core {
	...
	/**
	 * Run KO7 with backwards compatibility module "kohana"
	 * @var bool
	 */
	 ...
	 * ----------|------------|------------------------------------------------|---------------
	 * `string`  | base_url   | The base URL for your application.  This should be the *relative* path from your DOCROOT to your `index.php` file, in other words, if KO7 is in a subfolder, set this to the subfolder name, otherwise leave it as the default.  **The leading slash is required**, trailing slash is optional.   | `"/"`
	 * `string`  | index_file | The name of the [front controller](http://en.wikipedia.org/wiki/Front_Controller_pattern).  This is used by KO7 to generate relative urls like [HTML::anchor()] and [URL::base()]. This is usually `index.php`.  To [remove index.php from your urls](tutorials/clean-urls), set this to `FALSE`. | `"index.php"`
	 * `string`  | charset    | Character set used for all input and output    | `"utf-8"`
	 * `string`  | cache_dir  | KO7's cache directory.  Used by [KO7::cache] for simple internal caching, like [Fragments](ko7/fragments) and **\[caching database queries](this should link somewhere)**.  This has nothing to do with the [Cache module](cache). | `APPPATH."cache"`
	 * `integer` | cache_life | Lifetime, in seconds, of items cached by [KO7::cache]         | `60`
	 * `boolean` | errors     | Should KO7 catch PHP errors and uncaught Exceptions and show the `error_view`. See [Error Handling](ko7/errors) for more info. <br /> <br /> Recommended setting: `TRUE` while developing, `FALSE` on production servers. | `TRUE`
	 * `boolean` | profile    | Whether to enable the [Profiler](ko7/profiling). <br /> <br />Recommended setting: `TRUE` while developing, `FALSE` on production servers. | `TRUE`
	...
	public static function init(array $settings = NULL)
	{
		...
		{
			// Enable profiling
			KO7::$profiling = (bool) $settings['profile'];
		}
		...
		if (isset($settings['errors']))
		{
			...
			KO7::$errors = (bool) $settings['errors'];
		}
		 ...
		 */
		if (KO7::$environment == KO7::DEVELOPMENT AND extension_loaded('xdebug'))
		{
		...
		{
			KO7::$expose = (bool) $settings['expose'];
		}

		// Determine if we are running in a Windows environment
		KO7::$is_windows = (DIRECTORY_SEPARATOR === '\\');
		...
		if (isset($settings['cache_dir']))
		{
			if ( ! is_dir($settings['cache_dir']))
			{
				...
				{
					throw new KO7_Exception('Could not create cache directory :dir',
						[':dir' => Debug::path($settings['cache_dir'])]);
				}
			}

			// Set the cache directory path
			KO7::$cache_dir = realpath($settings['cache_dir']);
		}
		else
		{
			// Use the default cache directory
			KO7::$cache_dir = APPPATH.'cache';
		}

		if ( ! is_writable(KO7::$cache_dir))
		{
			throw new KO7_Exception('Directory :dir must be writable',
				[':dir' => Debug::path(KO7::$cache_dir)]);
		}

		if (isset($settings['cache_life']))
		{
			// Set the default cache lifetime
			KO7::$cache_life = (int) $settings['cache_life'];
		}

		if (isset($settings['caching']))
		{
			// Enable or disable internal caching
			KO7::$caching = (bool) $settings['caching'];
		}

		if (KO7::$caching === TRUE)
		{
			// Load the file path cache
			KO7::$_files = KO7::cache('KO7::find_file()');
		}

		if (isset($settings['charset']))
		{
			// Set the system character set
			KO7::$charset = strtolower($settings['charset']);
		}

		if (function_exists('mb_internal_encoding'))
		{
			// Set the MB extension encoding to the same character set
			mb_internal_encoding(KO7::$charset);
		}

		if (isset($settings['base_url']))
		{
			// Set the base URL
			KO7::$base_url = rtrim($settings['base_url'], '/').'/';
		}

		if (isset($settings['index_file']))
		{
			// Set the index file
			KO7::$index_file = trim($settings['index_file'], '/');
		}

		// Sanitize all request variables
		$_GET    = KO7::sanitize($_GET);
		$_POST   = KO7::sanitize($_POST);
		$_COOKIE = KO7::sanitize($_COOKIE);

		// Load the logger if one doesn't already exist
		if ( ! KO7::$log instanceof Log)
		{
			KO7::$log = Log::instance();
		}

		// Load the config if one doesn't already exist
		if ( ! KO7::$config instanceof Config)
		{
			KO7::$config = new Config;
		}
	}
	...
	public static function modules(array $modules = NULL)
	{
		...
		foreach ($modules as $name => $path)
		{
			...
			else
			{
				...
				throw new KO7_Exception('Attempted to load an invalid or missing module \':module\' at \':path\'', [
					':module' => $name,
		...
		KO7::$_modules = $modules;

		foreach (KO7::$_modules as $path)
		{
			$init = $path.'init'.EXT;
    ...
    public static function file_cache($name, $data = NULL, $lifetime = NULL)
    {
        ...
        $dir = KO7::$cache_dir.DIRECTORY_SEPARATOR.$file[0].$file[1].DIRECTORY_SEPARATOR;
