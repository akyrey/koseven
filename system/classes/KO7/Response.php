...
class KO7_Response implements HTTP_Response {
	...
	public function send_file($filename, $download = NULL, array $options = NULL)
	{
		...
		if ($filename === TRUE)
		{
			if (empty($download))
			{
				throw new KO7_Exception('Download name must be provided for streaming files');
			}
		...
		else if (is_resource($filename) && get_resource_type($filename) === 'stream')
		{
			...
			{
				throw new KO7_Exception('Resource must be a file handle');
			}
		...
		if ( ! is_resource($file))
		{
			throw new KO7_Exception('Could not read file to send: :file', [
				':file' => $download,
			]);
		}
